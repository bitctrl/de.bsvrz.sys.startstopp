...
Die Oberfläche wird je nach Bedarf und Betriebssystem in einem eigenen Fenster gestartet oder direkt innerhalb der Shell, in dem sie aufgerufen wurde ausgeführt. Es sollte daher immer eine eigene Shell verwendet werden, damit potentielle Ausgaben anderer Prozesse die Oberfläche nicht beeinflussen und die Darstellung stören können.
...
* *STOPPED:* die StartStopp-Konfiguration wurde angehalten, die SWE slebst ist abr noch aktiv
...
Der Modus des Betriebsmeldungsversandes zeigt an, ob beim Starten und Stoppen von Applikationen Betriebsmeldungen versandt werden sollen. Das erfordert natürlich eine bestehende Datenverteilerverbindung und eine aktive Betriebsmeldungsverwaltung.
...
* *System neu starten* ist eine Konbination aus den beiden oben genannten Funktionen. Zunächst wird die StartStopp-Konfiguration angehalten und dann wieder gestartet.
